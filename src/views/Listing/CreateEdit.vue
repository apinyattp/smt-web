  ...
  <form ref="formData" class="space-y-8" @submit.prevent="onSave">
    <div class="flex flex-col bg-gray-500">
        ...
        <div class="container mx-auto flex items-center">
          <div class="flex-grow">
            ...
            <!-- <router-link
              ...
              >Back</router-link
            > -->
            <div class="text-xl text-gray-200 mt-3">
              {{ isEdit ? 'แก้ไข' : 'เพิ่ม' }}โครงการ
            </div>
          </div>
      ...
      <div class="w-1/2 pb-20 mx-auto">
        ...
        <div class="font-medium text-gray-200 mb-2 col-span-2 mb-4">
          โครงการ
        </div>
        <div class="grid grid-cols-2 gap-y-8 gap-x-10">
          ...
          <div class="flex flex-col">
            ...
            <base-select
              v-model="type"
              :additional-select-class="
                errors['type'] ? 'mt-auto error' : 'mt-auto'
              "
          ...
          <div>
            ...
            <input
              ...
              type="text"
              :class="
                errors['location'] ? 'input-primary error' : 'input-primary'
              "
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input
              ...
              type="text"
              :class="
                errors['roomType'] ? 'input-primary error' : 'input-primary'
              "
            />
          </div>
          <div>
            ...
            <input
              ...
              type="text"
              :class="
                errors['subway'] ? 'input-primary error' : 'input-primary'
              "
            />
          </div>
          <div>
          ...
          <div class="col-span-2">
            ...
            <textarea
              ...
              type="text"
              :class="
                errors['description'] ? 'input-primary error' : 'input-primary'
              "
              rows="13"
            />
          </div>
          <div class="col-span-2">
            ...
            <input
              ...
              type="text"
              :class="
                errors['original'] ? 'input-primary error' : 'input-primary'
              "
            />
          </div>
        </div>
        <div class="grid grid-cols-2 gap-y-8 gap-x-10">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              :class="
                errors['author'] ? 'input-primary error' : 'input-primary'
              "
            />
          </div>
          <div>
...
export default {
  ...
  setup(props, context) {
    ...
    const onSubmit = handleSubmit((submitData) => {
      const a_entity = {
        ...
        number_bedroom: submitData.roomType
          ? submitData.roomType.split(',')
          : '',
        investment: [],
        buy_phase: [],
        sell_phase: [],
  ...
  watch: {
    result(to) {
      const a_hilight = to.a_listing.hilight
        ? JSON.parse(to.a_listing.hilight)
        : []
      const a_predict = to.a_predict
      const a_listing_obj = to.a_listing.addObj
        ? JSON.parse(to.a_listing.addObj)
        : []
      const mapPredict = {
        ...
        roomType: a_predict.number_bedroom
      ...
      this.location = a_hilight.location
        ? a_hilight.location.join(', ')
        : mapPredict.location
      this.price = a_hilight.price
        ? a_hilight.price.join(', ')
        : mapPredict.price
      this.size = a_hilight.size ? a_hilight.size.join(', ') : mapPredict.size
      this.roomType = a_hilight.number_bedroom
        ? a_hilight.number_bedroom.join(', ')
        : mapPredict.roomType
      this.subway = a_hilight.station
        ? a_hilight.station.join(', ')
        : mapPredict.subway
